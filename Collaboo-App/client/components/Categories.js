...
class Categories extends Component {
  ...
  componentDidMount() {
    ...
    const data = {categories: this.state.categories};
    fetch(url,{
      method:"GET",
    })
  ...
  findCategory(query) {
    ...
    console.log("firstlog" + categories)
    ...
    return categories && categories.filter(categories => (categories.compname).search(regex) >= 0);
  }
  handleChange = (item) =>{
    // console.log(this.state)
    console.log(item)
    ...
    return (
      <View style={styles.container}>
        <Autocomplete
          ...
          data={categories && categories.length === 1 && comp(query, categories[0].compname) ? [] : categories}
